...
This project demonstrates how to decouple an AI agent's "brain" (LLM reasoning) from its "hands" (tools) using standardised MCP communication over Server-Sent Events (SSE).
...
2. **`mcp-agent-loop` (Orchestrator)**: A Python script utilising **LangChain**, **LangGraph**, and **LangChain MCP Adapters** to dynamically discover tools, process user requests, and orchestrate local LLM calls.
